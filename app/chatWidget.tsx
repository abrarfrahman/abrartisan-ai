        ...
        <div className={`bg-white rounded-lg shadow-lg overflow-hidden ${showFullScreen ? 'w-full h-full fixed top-0 left-0 m-0' : 'w-[420px]'}`}>
            ...
            <div className="px-4 py-3 border-t flex items-center justify-between">
                ...
                <DropdownMenuTrigger asChild>
                    <Button variant="outline" size="sm" className={`gap-1 ${getContextColor(context)}`}>
                    {context}
                    <ChevronDown className="h-4 w-4" />
                    </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="end">
                    <DropdownMenuItem onClick={() => setContext('Onboarding')} className={getContextColor('Onboarding')}>
                    Onboarding
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => setContext('Campaign')} className={getContextColor('Compliance')}>
                    Campaign
                    </DropdownMenuItem>
                </DropdownMenuContent>
                </DropdownMenu>
